...
basepath_basenorm_feats <- "/nas/longleaf/home/adaigle/work/test_TEforest/test_lightgbm_"

read_df <- function(basepath, covg, genome) {
    path <- paste0(basepath, covg, "X_newfilter_dynamiclength/2L_2R_plots/performance_plots/")
...
read_df_trim <- function(basepath, covg, genome) {
    ...
    df <- df %>%
        mutate(
            caller = case_when(
                caller == "TEforest_new_trimmed_bps" ~ "TEforest",
                caller == "temp" ~ "TEMP",
                caller == "temp2" ~ "TEMP2",
                caller == "teflon" ~ "TEFLoN",
                caller == "retroseq" ~ "RetroSeq",
...
plot_coverage_threeplot <- function(df) {
  ...
  for (metric in metrics) {
    ...
    present <- intersect(names(colors), unique(d$caller))
...
##  )
##
##ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/reference_figure.png", reference_figure, dpi=300, width = 8.5, height = 10.5)
##ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/reference_figure.svg", reference_figure, dpi=300, width = 8.5, height = 10.5)
##ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/reference_figure.tiff", reference_figure, dpi=300, width = 8.5, height = 10.5)
...
#load('/nas/longleaf/home/adaigle/work/test_TEforest/basenorm_feats_50X_newfilter_dynamiclength/2L_2R_plots/JUT-008_MUN-009.RData')


read_commoncaller_csv <- function(basepath, covg, genome, num_callers) {
    path <- paste0(basepath, covg, "X_newfilter_dynamiclength/2L_2R_plots/")
...
fn_data <- data.frame(
  ...
  FN_candidate = c(74, 72, 75),   # FN in candidate regions stage
  FN_class = c(24, 27, 23),       # FN in classification stage
  TP = c(314, 266, 300)           # TP in classification stage
)
...
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/merged_performance.svg", final_with_col_labels, dpi=300, width = 8.5, height = 9)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/merged_performance.jpg", final_with_col_labels, dpi=300, width = 8.5, height = 9)

ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/classify_falsenegatives.svg", stacked_bar, dpi=300, width = 8.5, height = 8.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/classify_falsenegatives.jpg", stacked_bar, dpi=300, width = 8.5, height = 8.5)

merged_breakpoints <- ggarrange(
    breakpoints_generic_150, breakpoints_generic_125, breakpoints_generic_54, 
    breakpoints_3commoncallers_150, breakpoints_3commoncallers_125, breakpoints_3commoncallers_54, 
    labels = c("A", "B", "C", "D", "E", "F"),
    ncol = 3, nrow = 2, common.legend = T, legend="bottom"
  )
merged_breakpoints_with_col_labels <- ggarrange(
  titles_row, merged_breakpoints,
  ncol = 1, heights = c(0.06, 1)  # tweak the header height as you like
)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/merged_breakpoints.svg", merged_breakpoints_with_col_labels, dpi=300, width = 8.5, height = 8)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/merged_breakpoints.jpg", merged_breakpoints_with_col_labels, dpi=300, width = 8.5, height = 8)

geno_3commoncallers_150 <- basenorm_feats_common_calls_154bp_three %>%
  filter(!covg %in% c(5, 10)) %>%
  plot_genotyping()

geno_3commoncallers_125 <- basenorm_feats_common_calls_125bp_three %>%
  filter(!covg %in% c(5, 10)) %>%
  plot_genotyping()

geno_3commoncallers_54 <- basenorm_feats_common_calls_54bp_three %>%
  filter(!covg %in% c(5, 10)) %>%
  plot_genotyping()
...
read_genotype_data <- function(basepath, covg, genome) {
    ...
    for (coverage in covg) {
        ...
        path <- paste0(basepath, coverage, "X_newfilter_dynamiclength/2L_2R_plots/")
...
plot_combined <- function(df) {
    ...
    df <- df %>%
        mutate(jittered_covg = jitter(covg, amount = 1.2))  # Jitter the x-axis (covg) values
    df$caller <- factor(df$caller, levels = names(colors))
    df <- df[order(df$caller, decreasing = TRUE), ]
    ggplot(df) +
        # Plot for Tphomo
...
#combined_plot_legend <- get_legend(combined_plotwlegend + theme(legend.position = "bottom"))
combined_plot_legend <- ggpubr::get_legend(combined_plot+ theme(legend.position = "bottom"))  # if you still get the warning, try return_all=TRUE

# 1) Extract the legend specifically from panel I for each coverage
...
combined_plot_legend_54 <- ggpubr::get_legend(
  combined_plot_54 +
    theme(legend.position = "bottom",
          legend.title = element_blank())
)

# 2) Build the 125 figure, forcing ggarrange to use that legend grob
genotyping_figure_125 <- ggpubr::ggarrange(
  frqplt_teforest_125 + theme(legend.position = "none"),
  frqplt_retroseq_125 + theme(legend.position = "none"),
  frqplt_temp2_125 + theme(legend.position = "none"),
  all_data_125$TEforest_conf_matrix_30X + theme(legend.position = "none"),
  all_data_125$retroseq_conf_matrix_30X + theme(legend.position = "none"),
  all_data_125$temp2_conf_matrix_30X + theme(legend.position = "none"),
  alldata_geno_125 + theme(legend.position = "none"),
  geno_3commoncallers_125 + theme(legend.position = "none"),
  combined_plot_125 + theme(legend.position = "none"),  # legend extracted already
  labels = c("A","B","C","D","E","F","G","H","I"),
  ncol = 3, nrow = 3,
  common.legend = TRUE,
  legend = "bottom",
...
# 3) Build the 54 figure the same way
genotyping_figure_54 <- ggpubr::ggarrange(
  frqplt_teforest_54 + theme(legend.position = "none"),
  frqplt_retroseq_54 + theme(legend.position = "none"),
  frqplt_temp2_54 + theme(legend.position = "none"),
  all_data_54$TEforest_conf_matrix_30X + theme(legend.position = "none"),
  all_data_54$retroseq_conf_matrix_30X + theme(legend.position = "none"),
  all_data_54$temp2_conf_matrix_30X + theme(legend.position = "none"),
  alldata_geno_54 + theme(legend.position = "none"),
  geno_3commoncallers_54 + theme(legend.position = "none"),
  combined_plot_54 + theme(legend.position = "none"),
  labels = c("A","B","C","D","E","F","G","H","I"),
  ncol = 3, nrow = 3,
  common.legend = TRUE,
  legend = "bottom",
  legend.grob = combined_plot_legend_54
)
genotyping_figure <- ggpubr::ggarrange(
  frqplt_teforest + theme(legend.position = "none"),
  frqplt_retroseq + theme(legend.position = "none"),
  frqplt_temp2 + theme(legend.position = "none"),
  all_data$TEforest_conf_matrix_30X + theme(legend.position = "none"),
  all_data$retroseq_conf_matrix_30X + theme(legend.position = "none"),
  all_data$temp2_conf_matrix_30X + theme(legend.position = "none"),
  alldata_geno + theme(legend.position = "none"),
  geno_3commoncallers_150 + theme(legend.position = "none"),
  combined_plot + theme(legend.position = "none"),  # legend already extracted
  labels = c("A","B","C","D","E","F","G","H","I"),
  ncol = 3, nrow = 3,
  common.legend = TRUE,
  legend = "bottom",
  legend.grob = combined_plot_legend                     # <-- this forces ggarrange to use panel I’s legend
)
# 4) Save
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_125.svg",
       genotyping_figure_125, dpi=300, width=8.5, height=8.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_125.jpg",
       genotyping_figure_125, dpi=300, width=8.5, height=8.5)

ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_54.svg",
       genotyping_figure_54, dpi=300, width=8.5, height=8.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_54.jpg",
       genotyping_figure_54, dpi=300, width=8.5, height=8.5)

ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_150.svg", genotyping_figure, dpi=300, width = 8.5, height = 8.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/genotyping_150.jpg", genotyping_figure, dpi=300, width = 8.5, height = 8.5)
...
#analyze genotype specific performance for reviewer
# 1. Calculate Homozygous Recall (Sensitivity)
# Meaning: What % of True Homozygotes were found (regardless of whether called Hom or Het)?
combined_genotype_data$homo_detection_recall <- (combined_genotype_data$homo_tp_length + combined_genotype_data$homo_fp_het_length) / 
                            (combined_genotype_data$homo_tp_length + combined_genotype_data$homo_fp_het_length + combined_genotype_data$homo_fn_length)

# 2. Calculate Heterozygous Recall (Sensitivity)
# Meaning: What % of True Heterozygotes were found (regardless of whether called Het or Hom)?
combined_genotype_data$het_detection_recall <- (combined_genotype_data$het_tp_length + combined_genotype_data$het_fp_homo_length) / 
                           (combined_genotype_data$het_tp_length + combined_genotype_data$het_fp_homo_length + combined_genotype_data$het_fn_length)
combined_genotype_data %>% select(caller, homo_detection_recall, het_detection_recall)

#load('/nas/longleaf/home/adaigle/work/test_TEforest/basenorm_feats_40X_newfilter_dynamiclength/2L_2R_plots/AKA-017_GIM-024.RData')
...
basepath_trimmed_read_experiment <- "/nas/longleaf/home/adaigle/users/test_TEforest/trim_reads_test_newmodel"
...
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_150_plot.png", trimmed_reads_150_plot, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_150_plot.svg", trimmed_reads_150_plot, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_150_plot.tiff", trimmed_reads_150_plot, dpi=300, width = 8.5, height = 10.5)

ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_125_plot.png", trimmed_reads_125_plot, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_125_plot.svg", trimmed_reads_125_plot, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/trimmed_reads_125_plot.tiff", trimmed_reads_125_plot, dpi=300, width = 8.5, height = 10.5)
...
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_150_plot.png", bp_distribution_figure, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_150_plot.svg", bp_distribution_figure, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_150_plot.tiff", bp_distribution_figure, dpi=300, width = 8.5, height = 10.5)


ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_125_plot.png", bp_distribution_figure_125, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_125_plot.svg", bp_distribution_figure_125, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_125_plot.tiff", bp_distribution_figure_125, dpi=300, width = 8.5, height = 10.5)

ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_54_plot.png", bp_distribution_figure_54, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_54_plot.svg", bp_distribution_figure_54, dpi=300, width = 8.5, height = 10.5)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/bp_distribution_figure_54_plot.tiff", bp_distribution_figure_54, dpi=300, width = 8.5, height = 10.5)
