...
mapping_results_filter <- mapping_results %>% mutate(
    ...
    euchromatin_calls_not_inrefte_reshrink = map(euchromatin_calls_not_inrefte_expand, # toggle numbers in ranges to change expansion
        ~ GRanges(seqnames = seqnames(.x), 
            ranges = IRanges(start = start(.x) + 200, end = end(.x) - 200))), #expand to capture deleted ref tes
    #keep only filtered regions with split reads
    regions_with_split_reads2 = map2(euchromatin_calls_not_inrefte_reshrink, regions_with_split_reads,  
        ~ subsetByOverlaps(.x,.y,ignore.strand = TRUE)),
    euchromatin_calls_wsplitreads = map(regions_with_split_reads2, ~ subsetByOverlaps(.x, euchromatin_coordinates, ignore.strand = TRUE)),
    ...
    expand1 = map(euchromatin_calls_without_nested_reftes, # toggle numbers in ranges to change expansion
        ~ GenomicRanges::reduce(GRanges(seqnames = seqnames(.x), 
            ranges = IRanges(start = start(.x) - 400, end = end(.x) + 400)))),
    expand2 = map(expand1, # toggle numbers in ranges to change expansion
        ~ GRanges(seqnames = seqnames(.x), 
            ranges = IRanges(start = start(.x) + 400, end = end(.x) - 400))),
    expand3 = map(expand2, 
        ~ subset(.x, width(.x) >= 250)),
    expand = map(expand3, # toggle numbers in ranges to change expansion
        ~ GenomicRanges::reduce(GRanges(seqnames = seqnames(.x), 
            ranges = IRanges(start = start(.x) - 200, end = end(.x) + 200)))),
)
