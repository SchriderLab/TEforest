...
nonreference_genome2_truth <- nonreference_genome2_truth  #%>% filter(seqnames %in% c("2R","3R")) #CHANGED
...
reference_df_R <- reference_df  %>% filter(V1 %in% c("2R","3R", "2L", "3L", "X")) #CHANGED
...
reference_df_labled <- rbind(reference_df_R) %>% arrange(V1) %>% #CHANGED
