...
# Specify the destination directory
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/synthetic_hets_mcclintock/", genome, "_1/results/", caller_name2)
if (!dir.exists(destination_folder)) {
  # Create the directory
  dir.create(destination_folder)
}
...
# Specify the destination directory
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/synthetic_hets_mcclintock/", genome, "_1/results/", caller_name2, "_bps")
if (!dir.exists(destination_folder)) {
  # Create the directory
  dir.create(destination_folder)
}
...
nonreference_genome1_truth_r <- nonreference_genome1_truth  %>% filter(seqnames %in% c("2R","3R", "2L","3L", "X"))
#nonreference_genome1_truth_notr <- nonreference_genome1_truth %>% filter(seqnames %in% c())
...
truth <- rbind(heterozygosity_2R3R)
...
training_csv_negative_downsampled <- training_csv_negative %>% separate(file, into = c("genome", "seqnames", "Start", "End", "truth", "TE"), sep = "-") %>% 
...
benchmark_mapping_results_frequency_plot <- benchmark_mapping_results_frequency_plot %>% mutate(
  tn_df = map(true_negatives, # reduce to get rid of nested TEs of same type
        ~ as.data.frame(.x)),
  tn = map2(caller, tn_df, ~ .y %>% mutate(caller = .x, truth = 0, heterozygosity=0, true = NULL, pred=NULL, cntrl_score=NULL, genome=NULL)))
