...
#genomes <- "A2_A3"
#csv_path <- "/nas/longleaf/home/adaigle/work/test_TEforest/full_model_no75bpredo/2L2R/A2_A3.csv"
#results_path <- "/nas/longleaf/home/adaigle/work/test_TEforest/full_model_no75bpredo/2L2R_model_validation_output/"
...
#find_breakpoint_tespecific <- function(candidate_region, genome) {
#    #print(candidate_region)
## shrink candidate region down to avoid spurious split reads
##based on original size of region
#shrink_size <- (width(candidate_region) / 1.1)/2
#candidate_region <- shrink(candidate_region, shrink_size, shrink_size)
#tesbp <- ScanBamParam(which = IRangesList(c(candidate_region)), what = c("qname", "seq"))
#bamfile <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/find_candidate_regions/rework_pipeline/outputs/v5_tests/", genome, "/", candidate_region$TE, "_to_ISO1.bam")
#tebam <- BamFile(bamfile)
#tereads <- readGAlignments(tebam, param = tesbp)
#
## Regular expression to match cigars ending in "S" or starting with a digit followed by "S"
...
#csv_path <- paste0(basedir_outputs_path, "/output/", genome, "/predictions.csv")
