...
-g <reference_genome.fasta> -@ <threads> -1 <R1.fastq.gz> -2 <R2.fastq.gz> -n <genome_name>"
    exit 1
}
[[ $# -eq 0 ]] && usage

while getopts o:c:@:g:1:2:r:n: flag; do
    case $flag in
...
for f in "$fasta_file" "$ref_genome" "$fq1" "$fq2" "$ref_te_locations"; do
...
###############################################################################
# 1. Build TE reference (consensus + genomic TE copies)
###############################################################################
awk '
    NF >= 3 && $2 ~ /^[0-9]+$/ && $3 ~ /^[0-9]+$/ {
...
| bedtools getfasta -fi "$ref_genome" -bed - \
        -fo "$output_directory/extracted_sequences.fasta" -nameOnly

cat "$output_directory/extracted_sequences.fasta" "$fasta_file" > \
    "$output_directory/te.tmp.fasta"
sed 's/-/_/g' "$output_directory/te.tmp.fasta" > \
    "$output_directory/te.fasta"
...
align_family() {
    ...
    allbam="$output_directory/${genome_name}_to_alltes.bam"
    contigs_file="$tmp/${name}.contigs"

    ########################################################################
    # 1st pass – build an in‑memory list of QNAMEs that touch the family
    ########################################################################
    # write the contig list
    printf '%s\n' "${contigs[@]}" > "$contigs_file"

    # build QNAMEs in a single shell (no subshell!)
    declare -A keep
    while read -r q; do keep["$q"]=1; done < <(
      samtools view -@ "$family_threads" -h -R "$contigs_file" "$allbam" |
      awk '$1 !~ /^@/ { print $1 }'
    )
    # dump out as FASTA‑like name file for samtools -N
    printf '%s\n' "${!keep[@]}" > "$tmp/${name}.qnames"

    # now one stream: pull all reads with those QNAMEs, convert to FASTQ, remap,
    # filter for discordant/split, sort → final BAM
    samtools view -@ "$family_threads" -h -n "$tmp/${name}.qnames" "$allbam" \
      | samtools sort -n -@ "$family_threads" -T "$tmp/${name}.nsort" -O BAM - \
      | samtools fastq -@ "$family_threads" \
                      -n \                           # keep QNAMEs identical
                      -                    \       # read BAM from stdin
      | bwa-mem2 mem -p -t "$family_threads" "$ref_genome" - \
      | gawk 'BEGIN{FS=OFS="\t"}
               /^@/ {print; next}
               {
                 flag = $2+0
                 proper = ((flag % 4) >= 2)
                 sup    = ((flag % 4096) >= 2048)
                 clip   = ($6 ~ /[0-9][SH]/) || index($0,"SA:Z:")
                 if (!proper || clip || sup) print
               }' \
      | samtools view -b -o "$output_directory/${name}_to_ISO1.bam" -
    samtools index "$output_directory/${name}_to_ISO1.bam"

#    ###########################################################################
#    # b)  BAM with hits on those contigs
#    ###########################################################################
#    # write the list of contigs to a temp file (one contig per line)
#    printf '%s\n' "${contigs[@]}" > "$tmp/${name}.contigs"
#    
#    # now let samtools read the regions from that file instead of on the CLI
#    samtools view -@ "$family_threads" -b -R "$tmp/${name}.contigs" \
