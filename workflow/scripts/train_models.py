...
df['read_length'] = df['files'].apply(lambda x: 151 if str(x).startswith('JUT') else (75 if str(x).startswith('RAL') else (125 if str(x).startswith('AKA') else (54 if str(x).startswith('A2') else None))))
...
df['insert_size'] = df['files'].apply(lambda x: 415 if str(x).startswith('JUT') else (220 if str(x).startswith('RAL') else (208 if str(x).startswith('AKA') else (287 if str(x).startswith('A2') else None))))
...
full_df = full_df.merge(df[['read_length']], left_index=True, right_index=True, how='left')
full_df = full_df.merge(df[['insert_size']], left_index=True, right_index=True, how='left')
...
s = setup(full_df, target = 'true', session_id = 123,html=False, use_gpu=True)#, fix_imbalance=True)#, normalize = True)
...
predicted_values['Correct'] = predicted_values['true'] == predicted_values['prediction_label']
