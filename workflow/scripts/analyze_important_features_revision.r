...
feature_data <- read.csv("/nas/longleaf/home/adaigle/TEforest/revised_plots/predictions_with_features.csv")
...
TE_orphan_sd_plt <- ggplot(data = feature_data, aes(x = TE_specific_Orphan_Read_IQR, y = true)) +
...
feat_names <- if (is.data.frame(feature_list_extended_df)) {
  as.character(feature_list_extended_df[[1]])
} else {
  as.character(feature_list_extended_df)
}

## 2) Locate and order the feature columns numerically
feat_cols <- grep("^feature_\\d+$", names(feature_data))
feat_cols <- feat_cols[order(as.integer(sub("^feature_", "", names(feature_data)[feat_cols])))]

## 3) Sanity check
if (length(feat_cols) != length(feat_names)) {
  stop(sprintf("Found %d feature_* columns but %d provided names.",
               length(feat_cols), length(feat_names)))
}

## 4) (Optional) Make names unique if duplicates exist
# if (anyDuplicated(feat_names)) feat_names <- make.unique(feat_names)

## 5) Rename
names(feature_data)[feat_cols] <- feat_names

feature_data_filter <- feature_data %>%
  filter(TE_specific_Orphan_Read_IQR >= 0, TE_specific_Orphan_Read_IQR <= 1)

TE_orphan_sd_plt_complex <- ggplot(data = feature_data_filter, aes(x = TE_specific_Orphan_Read_IQR, y = class)) +
...
TE_orphan_sd_plt_150 <- ggplot(data = feature_data_filter_150, aes(x = TE_specific_Orphan_Read_IQR, y = class)) +
...
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/TE_orphan_sd_plt_150.png", plot = TE_orphan_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/TE_orphan_sd_plt_150.svg", plot = TE_orphan_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/TE_orphan_sd_plt_150.tiff", plot = TE_orphan_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)


Cigar4_sd_plt_150 <- ggplot(data = feature_data_filter_150, aes(x = Cigar4_sd, y = class)) +
  geom_boxplot(aes(color = factor(pred)), width = 0.7, show.legend = TRUE) +
  labs(
    x = "Cigar4 sd",
    y = "Classification",
...
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/Cigar4_sd_plt_150.png", plot = Cigar4_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/Cigar4_sd_plt_150.svg", plot = Cigar4_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)
ggsave("/nas/longleaf/home/adaigle/TEforest/revised_plots/Cigar4_sd_plt_150.tiff", plot = Cigar4_sd_plt_150, width = 8.5, height = 8.5, dpi = 300)
...
data <- read.table("/nas/longleaf/home/adaigle/work/test_TEforest/test_lightgbm_50X_newfilter_dynamiclength/3L3RX/feature_importances_all.txt", header=T, sep="\t")
...
top_features <- data %>%
  ...
  select(
    ...
    `Impurity-Based Importance` = Built.in.Importance..gain.
  )
...
output_path <- "/nas/longleaf/home/adaigle/TEforest/revised_plots/feat_importance_50X.png"
