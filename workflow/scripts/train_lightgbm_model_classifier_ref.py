...
def train_rf(X_train, y_train, class_lab, outdir):
    """
    Fits a **LightGBM** classifier to data, dumps to disk.
    """
    clf = LGBMClassifier(
        n_estimators=500,
        random_state=42,
...
def all_runner(samps, data_classes, outdir):
    ...
    # ─── INSERT DOWNSAMPLING HERE ────────────────────────────────────────────
    #from sklearn.utils import resample
#
    ## Assume your classes are 1 (minority) vs. 2 (majority):
    #maj_label = 2
    #min_label = 1
#
    ## Make sure X_train and y_train are lists for easy slicing
    #X_train = list(X_train_arr)
    #y_train = list(y_train)
#
    ## Split into majority/minority
    #X_maj = [x for x, y in zip(X_train, y_train) if y == maj_label]
    #y_maj = [maj_label] * len(X_maj)
    #X_min = [x for x, y in zip(X_train, y_train) if y == min_label]
    #y_min = [min_label] * len(X_min)
#
    ## Down-sample the majority class to the minority’s size
    ...
    #)
#
    ## Rebuild a balanced train set and shuffle it
    #X_train_arr, y_train = resample(
    #    X_maj_down + X_min,
    #    y_maj_down + y_min,
    #    random_state=42
    #)
    ...
    # now train on the balanced set
    rf = train_rf(X_train_arr, y_train, "all", outdir)
    y_test, preds = pred_all_rf(rf, X_test_arr, y_test, files_test, data_classes, outdir)
    return y_test, preds
