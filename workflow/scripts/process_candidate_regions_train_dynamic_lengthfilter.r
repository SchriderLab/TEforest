...
mapping_results_filter <- mapping_results %>% mutate(
  ...
  euchromatin_calls_not_inrefte_expand = map(euchromatin_calls,
  ...
  expand1 = map(early_width_filter,
      ~ GenomicRanges::reduce(GRanges(seqnames = seqnames(.x), 
...
print(mapping_results_filter %>% select(TE, read_length, width_threshold))
