...
class SVMaker:
    ...
    def _create_sum_stats(self, summ_arr: arr) -> arr:
        ...
        for feat_ind in range(0, summ_arr.shape[0]):
            ...
            vec_ind = feat_ind * 4
            feat_vec[vec_ind + 0] = np.mean(_stand_feat)
            feat_vec[vec_ind + 1] = np.median(_stand_feat)
            feat_vec[vec_ind + 2] = np.std(_stand_feat)
            feat_vec[vec_ind + 3] = stats.iqr(_stand_feat, interpolation="midpoint")
...
class ReadMapper:
    ...
    def map_reads(self, reads_df: df) -> Union[None, arr]:
        """
