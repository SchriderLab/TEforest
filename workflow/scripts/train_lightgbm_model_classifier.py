...
#    return samps
def load_data(infile, outdir, overwrite=False):
    backup = os.path.join(outdir, "data_backup.pkl")
    if not os.path.exists(backup) or overwrite:
        # Load the three arrays
        sv     = np.load(infile)
        X      = sv["X"]        # shape (n_samples, n_features)
        files  = sv["files"]    # shape (n_samples,)
        labels = sv["labels"]   # shape (n_samples,)

        # Build the samps dict by zipping
        samps = {
            "data":  [X[i]      for i in range(len(files))],
            "files": [files[i]  for i in range(len(files))],
            "labs":  [labels[i] for i in range(len(files))],
        }

        # Cache it so future runs are fast
        with open(backup, "wb") as f:
            pkl.dump(samps, f)
    else:
        # load from pickle if already done
...
def train_rf(X_train, y_train, class_lab, outdir, feature_names=None, built_in_type="gain"):
    """
    Fits a LightGBM classifier, saves model, and writes a table with
    built-in and permutation feature importances.

    built_in_type: "gain" or "split"
    """
    # Choose objective based on labels
    ...
    # --- Built-in importances (LightGBM) ---
    # Option A: via sklearn wrapper (split counts)
    built_in_split = clf.feature_importances_

    # Option B: via booster (gain or split)
    # Note: importance_type="gain" is typically more informative.
    if built_in_type not in {"gain", "split"}:
        built_in_type = "gain"
    built_in_importances = clf.booster_.feature_importance(importance_type=built_in_type)

    # --- Permutation importances ---
    # (run on training data to mirror your RF code; consider using a held-out set in practice)
    perm = permutation_importance(
        clf, X_train, y_train, n_repeats=10, random_state=42, n_jobs=-1
    )
    perm_importances = perm.importances_mean
    perm_std = perm.importances_std

    # --- Prepare output ---
    # Sort by built-in importance (like your RF baseline)
    indices = np.argsort(built_in_importances)[::-1]

    # Human-friendly feature labels
    if feature_names is None:
        # fall back to numeric indices
        feature_labels = [str(i) for i in range(len(built_in_importances))]
    else:
        feature_labels = list(feature_names)

    outpath = os.path.join(outdir, f"feature_importances_{class_lab}.txt")
    with open(outpath, "w") as f:
        f.write(
            "Feature\tBuilt-in Importance"
            f" ({built_in_type})\tBuilt-in (split)\tPermutation Importance\tPermutation Std\n"
        )
        for i in indices:
            f.write(
                f"{feature_labels[i]}\t"
                f"{float(built_in_importances[i]):.6f}\t"
                f"{float(built_in_split[i]):.6f}\t"
                f"{float(perm_importances[i]):.6f}\t"
                f"{float(perm_std[i]):.6f}\n"
            )

    print(f"Feature importances saved to {outpath}")
    return clf
