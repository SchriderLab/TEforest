...
#plt_dir <- "/nas/longleaf/home/adaigle/work/test_TEforest/nolengthfilter_30X/2L_2R_plots"
#basedir_outputs_path <- "/nas/longleaf/home/adaigle/work/test_TEforest/nolengthfilter_30X"
#coverage <- "30"
...
#result_path <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage,  "X/", genome, "_1/results/")
#result_path <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage, "X/", genome, "_1/results/")
result_path <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage, "X/", genome, "_1/results/")
...
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage,  "X/", genome, "_1/results/", caller_name)
...
# Specify the destination directory
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage,  "X/", genome, "_1/results/", caller_name, "_bps")
if (!dir.exists(destination_folder)) {
  # Create the directory
  dir.create(destination_folder)
}
...
# Specify the destination directory
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage,  "X/", genome, "_1/results/", caller_name2)
if (!dir.exists(destination_folder)) {
  # Create the directory
  dir.create(destination_folder)
}
...
# Specify the destination directory
destination_folder <- paste0("/nas/longleaf/home/adaigle/work/mcclintock_stuff/coverage_tests_ref/", coverage,  "X/", genome, "_1/results/", caller_name2, "_bps")
if (!dir.exists(destination_folder)) {
  # Create the directory
  dir.create(destination_folder)
}
...
het_conf_matrix <- benchmark_mapping_results_summary %>%
  rowwise() %>%
  mutate(
    conf_matrix = list(matrix(c(true_negatives_length, het_fn_length + het_fp_zero_length, homo_fn_length + homo_fp_zero_length,
...
cm <- cm[[1]]
# Calculate row-wise proportions for coloring
cm_proportion <- cm / rowSums(cm)
...
confusion_matrix <- ggplot(cm_combined, aes(x = Predicted, y = Actual, fill = Proportion)) +
  ...
  geom_text(aes(label = Value), vjust = 1, size = 5) +
  labs(x = "True Genotype", y = "Predicted Genotype", fill = "Row proportion") +
  theme_minimal() +
...
plt <- ggplot(benchmark_mapping_results_summary$distance_vectors_df[num][[1]],aes(x=distance)) + 
  ...
  labs(x = "Distance from true TE insertion (bp)", y = "Proportion of calls", title = caller_name) + 
